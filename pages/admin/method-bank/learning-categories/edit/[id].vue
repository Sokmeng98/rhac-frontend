<template>
	<h1 class="mb-10 text-4xl font-normal">Add PDF</h1>
...
const { updatePDF } = useLearningCategoriesStore()
...
const { data: pdfData } = (await useFetch(`/mb_professional_learning/${id}`, {
	baseURL
})) as any
...
const getUpdatedData = () => {
	...
	if (data.type !== type.value) formData.type = type.value
...
const onUpdatePdf = async () => {
	...
	if (hasNoDataChanged) navigateTo('/admin/method-bank/learning-categories')
	else {
