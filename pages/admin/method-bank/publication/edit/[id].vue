<template>
	<h1 class="mb-8 text-4xl font-normal">Edit PDF</h1>

	<section class="flex justify-end mb-6 space-x-3">
		<AdminCancelButton />
		<AdminActionButton text="save" @click="onUpdatePdf" />
	</section>
...
const error = ref({ pdfFile: false, title: false })

const { updatePDF } = usePublicationStore()
...
const { data: pdfData } = (await useFetch(`/publication/${id}`, { baseURL })) as any
const data = pdfData.value.data[0]

// Assign given data value to each fields
if (data) {
	pdfName.value = data.pdf.substring(data.pdf.lastIndexOf('/') + 1)
	if (data.image) thumbnailObjectURL.value = useFilePath(data.image)
...
const confirmCancel = () => {
	navigateTo('/admin/method-bank/publication')
}
...
const requiredFieldCheck = () => {
	if (!pdfFile.value && !pdfName.value) error.value.pdfFile = true
...
const getUpdatedData = () => {
	const formData = { _method: 'PUT' } as any

	if (pdfFile.value) formData.pdf = pdfFile.value
	if (thumbnailFile.value) formData.image = thumbnailFile.value

	if (data.title_en) {
		if (data.title_en !== titleEn.value) formData.title_en = titleEn.value
	} else if (titleEn.value) formData.title_en = titleEn.value

	if (data.title_kh) {
		if (data.title_kh !== titleKh.value) formData.title_kh = titleKh.value
	} else if (titleKh.value) formData.title_kh = titleKh.value

	return formData
}

const onUpdatePdf = async () => {
	requiredFieldCheck()
	if (error.value.pdfFile || error.value.title) return

	const updatedData = getUpdatedData()

	const hasNoDataChanged = Object.keys(updatedData).length === 1 && updatedData._method

	if (hasNoDataChanged) navigateTo('/admin/method-bank/publication')
	else {
		toast('pending', 'Updating PDF')
		const res = await updatePDF(data.id, updatedData)
		toastAfterFetch(res)
		toastShouldHide('pending')
		if (res.success) navigateTo('/admin/method-bank/publication')
	}
}
</script>
