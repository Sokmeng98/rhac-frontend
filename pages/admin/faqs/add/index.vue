<template>
	...
	<section class="space-x-3 text-right mt-7">
		...
		<AdminActionButton text="publish" @click="submitForm" />
...
import { QAndAForm, QAndAErrorFlags } from '~~/api/type'
...
const submitForm = async () => {
	...
	if (!errorMessageUnderButton.value) {
		toast('pending', 'Creating FAQ')

		const res = await createFaq(qAndA)

		toastAfterFetch(res)
		toastShouldHide('pending')

		if (res.success) {
			if (query.from === 'dashboard') navigateTo('/admin')
			else navigateTo('/admin/faqs')
		}
	}
}
</script>
