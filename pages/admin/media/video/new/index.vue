<template>
	<h1 class="mb-8 text-4xl font-normal">Add Video</h1>

	<section class="flex justify-end mb-6 space-x-3">
		<AdminCancelButton />
		<AdminActionButton text="publish" @click="submitVideoForm" />
	</section>
	...
	<section class="float-left -mt-9 p-1 w-[68%] overflow-hidden">
		...
		<div class="w-2/3">
			...
			<div
				v-if="!embedUrl"
				class="flex items-center justify-center border bg-neutral-50 aspect-video rounded-2xl"
			>
	...
	<!-- Categories Input -->
	<section class="bg-white rounded-md w-[30%] float-right">
...
import type { VideoForm } from '~~/api/type'
...
const dropdownArticle = ref(true)
const dropdownLearner = ref(true)
const dropdownProfessor = ref(true)
...
watch([titleEn, titleKh], () => {
	if (titleEn.value.trim().length || titleKh.value.trim().length) error.value.title = false
})
...
const submitVideoForm = async () => {
	...
	const data: VideoForm = { video_url: embedUrl.value }
	if (titleEn.value) data.title_en = titleEn.value
	if (titleKh.value) data.title_kh = titleKh.value
	if (post.value.length) data.post = post.value
	if (mbLearnerCategory.value.length) data.m_b__categories = mbLearnerCategory.value
	if (mbProfessional.value.length) data.mb_professional = mbProfessional.value

	toast('pending', 'Creating video')
